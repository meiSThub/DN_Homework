...
public class ScreenParamsUtils {
    ...
    private static ScreenParamsUtils mInstance;
    ...
    public static ScreenParamsUtils getInstance(Context context) {
        if (mInstance == null) {
            synchronized (ScreenParamsUtils.class) {
                if (mInstance == null) {
                    mInstance = new ScreenParamsUtils(context.getApplicationContext());
                }
            }
        }
        return mInstance;
    ...
    private ScreenParamsUtils(Context context) {
        //获取屏幕的真实宽高
     ...
     * @return
     */
    public int getFitWidth(float width) {
        return (int) (width * displayMetricsWidth / STANDARD_WIDTH);
    }

    public int getFitHeight(float height) {
